...
void MuonID_SS::initializeAnalyzer(){
	...
	if (DataYear == 2016) {
		...
		cerr << "[MuonID_SS::initializeAnalyzer] DataYear is wrong : " << DataYear << endl;
		exit(EXIT_FAILURE);
	}
	
	// ID settings
	MuonID_SSs = {"POGLoose", "POGTight"};
...
// ==== other functions ====
void MuonID_SS::ClearCollections() {
...
bool MuonID_SS::isSignal(const SR& sig) const {
	switch (sig) {
	case SR1:
	...
	case SR2:
		...
		break;
	default:
		cerr << "[MuonID_SS::isSignal] Wrong Signal region... sig = " << sig << endl;
		exit(EXIT_FAILURE);
	}
}

bool MuonID_SS::isControl(const CR& con) const {
	cout << "[MuonID_SS::isControl] Control region has not been set yet" << endl;
	exit(EXIT_FAILURE);
	...
	case CR2:
		...
		break;
	default:
		cerr << "[MuonID_SS::isControl] Wrong control region... con = " << con << endl;
		exit(EXIT_FAILURE);
	}
}
...
void MuonID_SS::DrawHists(TString path, const vector<Muon>& muons, const double& weight) {
	TString obj_path;
...
void MuonID_SS::DrawHists(TString path, const vector<Electron>& electrons, const double& weight) {
	TString obj_path;
...
void MuonID_SS::DrawHists(TString path, const vector<Jet>& jets, const double& weight) {
	TString obj_path;
...
void MuonID_SS::DrawHists(TString path, const vector<FatJet>& jets, const double& weight) {
	TString obj_path;
...
void MuonID_SS::DrawHists(TString path, const Particle& METv, const double& weight) {
