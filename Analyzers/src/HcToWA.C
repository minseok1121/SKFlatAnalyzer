...
void HcToWA::initializeAnalyzer(){
	...
	if (DataYear == 2016) {
		...
		cerr << "[HcToWA::initializeAnalyzer] DataYear is wrong : " << DataYear << endl;
		exit(EXIT_FAILURE);
	}
...
void HcToWA::executeEvent(){
	...
	if (nTight == 0) {
		...
		cerr << "[HcToWA] Sth wrong with muonID" << endl;
		exit(EXIT_FAILURE);
	}
...
bool HcToWA::isSignal(const SR& sig) const {
	switch (sig) {
	case SR1:
	...
	case SR2:
		...
		break;
	default:
		cerr << "[HcToWA::isSignal] Wrong Signal region... sig = " << sig << endl;
		exit(EXIT_FAILURE);
	}
}

bool HcToWA::isControl(const CR& con) const {
	cout << "[HcToWA::isControl] Control region has not been set yet" << endl;
	exit(EXIT_FAILURE);
	...
	case CR2:
		...
		break;
	default:
		cerr << "[HcToWA::isControl] Wrong control region... con = " << con << endl;
		exit(EXIT_FAILURE);
	}
}
...
void HcToWA::DrawHists(TString path, const vector<Muon>& muons, const double& weight) {
	TString obj_path;
...
void HcToWA::DrawHists(TString path, const vector<Electron>& electrons, const double& weight) {
	TString obj_path;
...
void HcToWA::DrawHists(TString path, const vector<Jet>& jets, const double& weight) {
	TString obj_path;
...
void HcToWA::DrawHists(TString path, const vector<FatJet>& jets, const double& weight) {
	TString obj_path;
...
void HcToWA::DrawHists(TString path, const Particle& METv, const double& weight) {
