...
// trigger & ID settings
    if (DataEra == "2016preVFP") {
        ...
        cerr << "[diLepRegion::initializeAnalyzer] Wrong era " << DataEra << endl;
        exit(EXIT_FAILURE);
    }
...
void diLepRegion::executeEvent(){
    ...
    for (const auto &syst: Systematics) {
        ...
        if (syst == "MuonMomentumShiftUp") { muon_coll = ScaleMuons(muon_coll, +1); }
        if (syst == "MuonMomentumShiftDown") {muon_coll = ScaleMuons(muon_coll, -1); }
        if (syst == "JetEnShiftUp") { jet_coll = ScaleJets(jet_coll, +1); }
        if (syst == "JetEnShiftDown") { jet_coll = ScaleJets(jet_coll, -1); }
        if (syst == "JetResShiftUp") { jet_coll = SmearJets(jet_coll, +1); }
        if (syst == "JetResShiftDown") { jet_coll = SmearJets(jet_coll, -1); }
        ...
        if (channel == "" || channel == "EMu/TTbarEnriched") return;
        ...
        if (channel == "DiMu/DrellYanEnriched") {
...
diLepRegion::channel_t diLepRegion::selectEvent(Event &ev,
...
double diLepRegion::getWeight(Event &ev,
                              vector<Muon> &muonT_coll,
                              vector<Muon> &muonL_coll,
                              vector<Muon> &muonV_coll,
                              vector<Electron> &eleT_coll,
    ...
    //    w_zpt = mcCorr->GetOfficialDYReweight(truth_coll);
    double w_toppt = mcCorr->GetTopPtReweight(truth_coll);
    ...
    if (syst == "Default") {
    ...
    else if (syst == "Central" || syst.Contains("Shift")) {
    }
        ...
        cerr << "[diLepRegion::getWeight] Wrong systematic " << syst << endl;
        exit(EXIT_FAILURE);
    }
	// electrons... not yet
    weight *= (w_prefire*w_gen*w_lumi*w_pileup);
    weight *= (w_idsf*w_trigsf);
    weight *= (w_zpt*w_toppt);
