...
void HcToWA_OSDimuon::initializeAnalyzer(){
	...
	if (DataYear == 2016) {
		...
		cerr << "[HcToWA_OSDimuon::initializeAnalyzer] DataYear is wrong : " << DataYear << endl;
		exit(EXIT_FAILURE);
	}
...
void HcToWA_OSDimuon::executeEvent(){
	...
	// OS dimuon
	// 1. pass trigger
	// 2. exactly one OS dimuon pair, no additional lepton
	// 3. pt > 20, 10
	...
	if (nTight == 0) {
		...
		cerr << "[HcToWA_OSDimuon] Sth wrong with muonID" << endl;
		exit(EXIT_FAILURE);
	}
...
// ==== other functions ====
void HcToWA_OSDimuon::ClearCollections() {
...
bool HcToWA_OSDimuon::isSignal(const SR& sig) const {
	switch (sig) {
	case SR1:
	...
	case SR2:
		...
		break;
	default:
		cerr << "[HcToWA_OSDimuon::isSignal] Wrong Signal region... sig = " << sig << endl;
		exit(EXIT_FAILURE);
	}
}

bool HcToWA_OSDimuon::isControl(const CR& con) const {
	cout << "[HcToWA_OSDimuon::isControl] Control region has not been set yet" << endl;
	exit(EXIT_FAILURE);
	...
	case CR2:
		...
		break;
	default:
		cerr << "[HcToWA_OSDimuon::isControl] Wrong control region... con = " << con << endl;
		exit(EXIT_FAILURE);
	}
}
...
void HcToWA_OSDimuon::DrawHists(TString path, const vector<Muon>& muons, const double& weight) {
	TString obj_path;
...
void HcToWA_OSDimuon::DrawHists(TString path, const vector<Electron>& electrons, const double& weight) {
	TString obj_path;
...
void HcToWA_OSDimuon::DrawHists(TString path, const vector<Jet>& jets, const double& weight) {
	TString obj_path;
...
void HcToWA_OSDimuon::DrawHists(TString path, const vector<FatJet>& jets, const double& weight) {
	TString obj_path;
...
void HcToWA_OSDimuon::DrawHists(TString path, const Particle& METv, const double& weight) {
...
HcToWA_OSDimuon::HcToWA_OSDimuon(){}
