...
void FakeEstimator_bak::initializeAnalyzer(){

	//==== Flag for systematic sources
	RunSysts = HasFlag("RunSysts");
	RunXsecSyst = HasFlag("RunXsecSyst");

	cout << "[FakeEstimator_bak::initializeAnalyzer] RunSysts = " << RunSysts << endl;
	cout << "[FakeEstimator_bak::initializeAnalyzer] RunXsecSyst = " << RunXsecSyst << endl;
	...
	if (DataYear == 2016) {
		...
		cout << "[FakeEstimator_bak::initializeAnalyzer] Trigger is not set for 2018" << endl;
		exit(EXIT_FAILURE);
	}
	else {
		cout << "[FakeEstimator_bak::initializeAnalyzer] Wrong Year" << endl;
		exit(EXIT_FAILURE);
	}

	cout << "[FakeEstimator_bak::initializeAnalyzer] HLTElecTriggerName = " << HLTElecTriggerName << endl;
	cout << "[FakeEstimator_bak::initializeAnalyzer] TriggerSafePtCut = " << TriggerSafePtCut << endl;
...
void FakeEstimator_bak::executeEvent(){
	...
	for (unsigned int i = 0; i < ElectronIDs.size(); i++) {
		...
		if (RunSysts) {
			for (unsigned int i = 1; i < AnalyzerParameter::NSyst; i++) {
				...
				//cout << "[FakeEstimator_bak::executeEvent] execute " << param.Name << endl;
				executeEventFromParameter(param);
			}
		}
		...
		//==== Xsec syst ====
		if (RunXsecSyst) {
			cout << "[FakeEstimator_bak::executeEvent] Xsec systematics is not set yet" << endl;
			exit(EXIT_FAILURE);
		}
	}			
}

void FakeEstimator_bak::executeEventFromParameter(AnalyzerParameter param){
	...
	if (electrons.size() == 1) {
		...
		//==== W boson dominated window
			...
			if (clean_jets04.at(0).Pt() > jetPtCuts.at(1)) {
				FillHist(Systs.at(1), "PromptNorm_W_enriched_" + param.Name, 1, weight, 2, 0., 2.);
			...
			if (clean_jets04.at(0).Pt() > jetPtCuts.at(2)) {
			    FillHist(Systs.at(2), "PromptNorm_W_enriched_" + param.Name, 1, weight, 2, 0., 2.);
				...
				if (IsDATA && NBjets_NoSF != 0) {
					FillHist(Systs.at(0), "PromptNorm_W_enriched_" + param.Name, 1, weight, 2, 0., 2.);
			...
			if (clean_jets04.at(0).Pt() > jetPtCuts.at(3)) {
				FillHist(Systs.at(3), "PromptNorm_W_enriched_" + param.Name, 1, weight, 2, 0., 2.);
			...
			if (clean_jets04.at(0).Pt() > jetPtCuts.at(4)) {
				FillHist(Systs.at(4), "PromptNorm_W_enriched_" + param.Name, 1, weight, 2, 0., 2.);
...
FakeEstimator_bak::FakeEstimator_bak(){
...
// Private Functions
double FakeEstimator_bak::GetNPVReweight(TString id, TString syst) {
...
double FakeEstimator_bak::GetCorrPt(Electron e) {
...
TH1D* FakeEstimator_bak::GetHist1D(TString suffix, TString histname) {
	TH1D* h = NULL;
...
TH2D* FakeEstimator_bak::GetHist2D(TString suffix, TString histname) {
	TH2D* h = NULL;
...
void FakeEstimator_bak::FillHist(TString syst, TString histname, double value, double weight, int n_bin, double x_min, double x_max) {
	TString id;
	for (unsigned int i = 0; i < ElectronIDs.size(); i++) {
...
void FakeEstimator_bak::FillHist(TString syst, TString histname, double value_x, double value_y, double weight,
...
void FakeEstimator_bak::WriteHist() {
	outfile->cd();
